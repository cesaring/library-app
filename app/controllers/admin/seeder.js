 ...
 //private methods       
  _saveRandomLibrary(isLast) {
            ...
            randomLibrary.save().then(() => {
                if (isLast) {
                    //down down via seeder-block to fader-label that we ready to show the label
...
_selectRandomLibrary() {
    ...
    //creaet a new arra from ids
    const libraryIds = libraries.map(lib=> lib.get('id'));
    
    //Randomly pick on id from the libaryIds array and return the library
